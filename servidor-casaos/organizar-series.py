...
PADROES = [
    re.compile(r"[Ss](\d{1,2})[\s._-]*[Ee](\d{1,3})(?:[\s._-]*[Ee](\d{1,3}))?"),  # S01E01, S01E01E02
...
def analisar(caminho: Path) -> dict | None:
    """Extrai série, temporada e episódio do nome do arquivo (ou da pasta pai)."""
    for origem in (caminho.stem, caminho.parent.name):
        for padrao in PADROES:
            m = padrao.search(origem)
            if not m:
                continue
            titulo = limpar_titulo(origem[: m.start()])
            if not titulo and origem is caminho.stem:
                titulo = limpar_titulo(caminho.parent.name)
            if not titulo:
                continue

            ano_m = ANO.search(origem) or ANO.search(caminho.parent.name)
            grupos = m.groups()
            return {
                "serie": titulo,
