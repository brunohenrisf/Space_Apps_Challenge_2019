...
export function signToken(payload: { sub: string; acc: string }, ttlSec = 86400): string {
  const body = { ...payload, exp: Math.floor(Date.now() / 1000) + ttlSec };
  const data = b64url(Buffer.from(JSON.stringify(body)));
  return `${data}.${hmac(data)}`;
}
export function verifyToken(token: string): { sub: string; acc: string; exp: number } | null {
...
export function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  (req as any).accountId = payload.acc;
  next();
}

// ---------- Seed do admin inicial ----------
export async function ensureAdmin(accountId: string): Promise<void> {
  if ((await store.countAdmins()) > 0) return;
  const email = config.adminEmail.toLowerCase().trim();
  await store.createAdmin(email, hashPassword(config.adminPassword), accountId);
